...
class FieldVisitor extends SimpleVisitor {
  FieldVisitor(this.typeMap, this.subTypeMap, this.operationName, this.graphqlTypeMeta,
      {Tap tap})
      : super(tap: tap) {
        schemaName = '$operationName${capitalizeUpperCase(graphqlTypeMeta.fieldName)}';
  ...
  List<FieldElement> visitField(FieldElement field) {
    if (field.selectionSet != null) {
      final childFiledsResults = OperationVisitor.generateFromSelection(
          schemaName, operationName, graphqlTypeMeta.name, field.selectionSet.selections, typeMap);
